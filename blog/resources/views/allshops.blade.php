        ...
        <div class="all-list-wrapper">
            ...
            <div class="menu-list-wrapper" id="divSortList">
                <ul id="sortListUl" class="list">
                    @if($cate_id==0)
                        <li sortid='0' class='current'><span class='category_name' category_id='0'>全部商品</span></li>
                    @else
                        <li sortid='0'><span class='category_name' category_id='0'>全部商品</span></li>
                    @endif
                    @foreach($category_name as $k=>$v)
                        @if($cate_id==$v->cate_id)
                            <li sortid='100+{{$k*5}}' reletype='1' linkaddr='' class='current'><span class='category_name' category_id='{{$v->cate_id}}'>{{$v->cate_name}}</span></li>
                        @else
                            <li sortid='100+{{$k*5}}' reletype='1' linkaddr=''><span class='category_name' category_id='{{$v->cate_id}}'>{{$v->cate_name}}</span></li>
                        @endif
                    @endforeach
                </ul>
            </div>
            ...
            <div class="good-list-wrapper">
                <div class="good-menu thin-bor-bottom">
                    <ul class="good-menu-list" id="ulOrderBy">
                        <li orderflag="10" class="current"><a class='type' where='0' href="javascript:;">默认</a></li>
                        <li orderflag="20"><a class='type' where='is_hot' href="javascript:;">人气</a></li>
                        <li orderflag="50"><a class='type' where='is_new' href="javascript:;">最新</a></li>
                        <li orderflag="30"><a class='type' where='desc' href="javascript:;">价值</a><span class="i-wrap"><i class="up"></i><i class="down"></i></span></li>
                        <!--价值(由高到低30,由低到高31)-->
                    </ul>
                </div>
                <div id='goods_data'>
                                    ...
                                    @foreach($goods_data as $v)
                                        <li id="23468">    
                                            <span class="gList_l fl">        
                                                <img class="lazy" data-original="{{url('goodsLogo\\')}}{{$v->goods_img}}">
                                            </span>    
                                            <div class="gList_r">        
                                                <h3 class="gray6">(第{{$v->goods_id}}云){{$v->goods_name}}{{$v->goods_desc}}</h3>        
...
<script>
    $(function(){
        ...
        $(document).on('click','.type',function(){
            var current=$(this).parent().addClass('current').siblings('li').removeClass('current');
            ...
            if(where=='desc'){
                ...
                $(this).attr('where','desc')
            }
            $('#type').val(where);
            $.get(
                "{{url('allshopsdo')}}",
                {contents:contents,type:where,content:content,category_id:category_id},
                function(res){
                    $('#goods_data').html(res);
                }
            )
